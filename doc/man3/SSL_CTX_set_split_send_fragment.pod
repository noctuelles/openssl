...
=head1 SYNOPSIS
 ...
 int SSL_set_record_size_limit(SSL_CTX *ctx, uint16_t limit);
...
functions will only accept a value in the range 64 - SSL3_RT_MAX_PLAIN_LENGTH.
...
SSL_CTX_set_record_size_limit() sets the maximum size of a protected record that the endpoint
is willing to receive.
This extension is advertise by default, and can be disabled by calling SSL_CTX_set_options(SSL_OP_NO_RECORD_SIZE_LIMIT_EXT).
If you set a maximum fragment length but do not explicitly set a record size limit, then the record size limit will
default to the same value as the maximum fragment length.
If you configure both maximum fragment length and record size limit, they will both be sent during
the handshake even if they have different values.
A compliant server will prioritize the record size limit over the maximum fragment length.
Both server and client can set this limit, but only client may initiate this extension use in the ClientHello.

See the B<Notes> to understand the key differences between maximum fragment length and record size limit.

If a maximum fragment length or record size limit is negotiated during the handshake, and the value set by
`SSL_CTX_set_max_send_fragment()` is *smaller* than either of those negotiated values,
then the `SSL_CTX_set_max_send_fragment()` value will be used as the actual limit for the size of sent fragments.
...
SSL_set_tlsext_max_fragment_length(), SSL_set_record_size_limit() fail if called on a QUIC SSL object.
...
A valid record size limit range is between 64 - SSL3_RT_MAX_PLAIN_LENGTH.
Record size limit does not apply to unprotected records. For (D)TLS 1.2 and below, initial handshake messages are not
affected by this limit unlike maximum fragment length. If you need this specific behavior, use maximum fragment length
instead.
Record size limit is asymmetric : a server can advertise a limit that is different than the client, and vise versa.
During renegotiation or resumption, the record size limit is renegotiated.
Records are subject to the limits that were set in the handshake that produces the keys that are used to protect those records.
This admits the possibility that the extension might not be negotiated when a connection is renegotiated or resumed.
